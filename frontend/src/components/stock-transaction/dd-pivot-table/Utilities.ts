...
const addSeparators = function (nStr: string | number, thousandsSep: string, decimalSep: string) {
...
export const numberFormat = function (opts_in?: any) {
    const defaults = {
...
export const sortAs = function (order: any[]) {
    const mapping: any = {};
    const l_mapping: any = {};
...
export const getSort = function (sorters: any, attr: string) {
    if (sorters) {
...
export const aggregatorTemplates: any = {
    ...
    uniques(fn: any, formatter = usFmtInt) {
        return ([attr]: any[]) => () => ({
            uniq: [] as any[],
            push(record: any) {
    ...
    fractionOf(wrapped: any, type = 'total', formatter = usFmtPct) {
        return (...x: any[]) => (data: any, rowKey: any[], colKey: any[]) => ({
...
aggregatorTemplates.var = (ddof: any, f: any) =>
    aggregatorTemplates.runningStat('var', ddof, f);
aggregatorTemplates.stdev = (ddof: any, f: any) =>
...
export class PivotData {
    ...
    constructor(inputProps = {}) {
        ...
        // (PropTypes as any).checkPropTypes(PivotData.propTypes, this.props, 'prop', 'PivotData');
    ...
    forEachMatchingRecord(criteria: any, callback: any) {
    ...
    arrSort(attrs: any[]) {
        const sortersArr = Array.from(attrs).map(a =>
            getSort(this.props.sorters, a)
        );
        return (a: any[], b: any[]) => {
    ...
    processRecord(record: any) {
        const colKey = Array.from(this.props.cols as any[]).map(x =>
            x in record ? record[x] : 'null'
        );
        const rowKey = Array.from(this.props.rows as any[]).map(x =>
            x in record ? record[x] : 'null'
        );
    ...
    static forEachRecord(input: any, derivedAttributes: any, f: any) {
        let addRecord;
