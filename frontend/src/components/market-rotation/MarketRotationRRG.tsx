...
export default function MarketRotationRRG() {
  ...
  const cachedPrefs = menuPreferencesService.getCachedPreferences(PAGE_ID) as RrgPreferences | null;
  ...
  const loadInputsIfNeeded = async (): Promise<LoadedInputsInfo> => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (viewMode === 'sector' && result.data.sectors) {
          const validItems = itemsToSelect.filter(item => result.data.sectors.includes(item));
          if (validItems.length !== itemsToSelect.length) {
            console.warn('⚠️ Frontend: Filtered out invalid sector items:', itemsToSelect.filter(item => !result.data.sectors.includes(item)));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6 overflow-x-auto">
          ...
          <div className="lg:col-span-3">
            <Card className="flex h-full flex-col">
              ...
              <CardContent className="flex-1 p-0" style={{ height: chartViewportHeight - 70 }}> {/* Minus header height approx */}
                {!hasRequestedData ? (
                  ...
                  <>
                        ...
                        {showSearchDropdown && (
                          <div className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-md shadow-lg z-50 max-h-60 overflow-y-auto">
                            {currentOptions.length === 0 ? (
                              ...
                              <>
                                {/* Show filtered results if searching, otherwise show all available */}
                                {(searchQuery ? (displayViewMode === 'sector' ? filteredSectorOptions : filteredStockOptions) : currentOptions.filter(option => !selectedItems.includes(option.name)))
                                ...
                                {/* Show "no results" message */}
                                {searchQuery && (displayViewMode === 'sector' ? filteredSectorOptions : filteredStockOptions).length === 0 && (
                                  <div className="p-2 text-sm text-muted-foreground">
                    ...
                    {selectedItems.length > 0 && (
                      <div>
                        ...
                        {selectedItems.length === 1 && (
                          <p className="text-xs text-muted-foreground mt-2">
                            At least one {displayViewMode === 'sector' ? 'sector' : 'stock'} must be selected for comparison
                          </p>
                        )}
                      </div>
                    )}
                              ...
                              {getFilteredStockScreenerOptions
                                ...
                                .map((option: any, index) => (
                                  <button
                                    ...
                                    className={`flex items-center justify-between w-full px-3 py-2 text-left hover:bg-accent transition-colors ${index === stockScreenerSearchDropdownIndex ? 'bg-accent' : ''
                                      }`}
                                  >
                                    <div className="flex items-center gap-2">
                              ...
                              {getFilteredSectorScreenerOptions
                                ...
                                .map((option: any, index) => (
                                  <button
                                    ...
                                    className={`flex items-center justify-between w-full px-3 py-2 text-left hover:bg-accent transition-colors ${index === sectorScreenerSearchDropdownIndex ? 'bg-accent' : ''
                                      }`}
                                  >
                                    <div className="flex items-center gap-2">
                                      <div className="w-3 h-3 rounded-full" style={{ backgroundColor: option.color }}></div>
