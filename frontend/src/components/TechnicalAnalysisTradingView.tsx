  ...
  useEffect(() => {
    ...
    try {
      ...
      if (style === 'line') {
        ...
        s.setData(filteredRows.map(d => ({ time: d.time, value: d.close })));
        priceRef.current = s;
      } else if (style === 'area') {
            ...
            if (param && param.time && chartRef.current) {
              ...
              try {
                ...
                if (chartRef.current.setCrosshairPosition) {
                  try {
                    chartRef.current.setCrosshairPosition(param.time, param.seriesData || {});
                  } catch (timeError) {
                    console.log('Time positioning failed on main chart, trying logical:', timeError);
                    try {
                      chartRef.current.setCrosshairPosition(param.logical, param.seriesData || {});
                    } catch (logicalError) {
  ...
  useEffect(() => {
    ...
    if (visibleRange) {
      const filteredData = footprintData.filter(data => 
        data.time >= visibleRange.from && data.time <= visibleRange.to
      );
    ...
    const unsubscribe = timeScale.subscribeVisibleTimeRangeChange((timeRange) => {
      ...
      if (timeRange) {
        const filteredData = footprintData.filter(data => 
          data.time >= timeRange.from && data.time <= timeRange.to
        );
        setVisibleData(filteredData);
      }
    });
