...
export class BrokerTransactionStockFDRGTNNGCalculator {
  ...
  private async processSingleDtFile(blobName: string, progressTracker?: ProgressTracker): Promise<{ success: boolean; dateSuffix: string; files: string[]; timing?: any; stockCount?: number }> {
    ...
    // Active date sudah di-set di generateBrokerTransactionStockFDRGTNNGData() SETELAH pre-check
    ...
    try {
      ...
      for (const type of types) {
        ...
        for (const investorType of investorTypes) {
          ...
          const result = await this.createBrokerTransactionPerStock(filtered, dateSuffix, type, investorType, progressTracker);
          ...
          console.log(`✅ Created ${result.files.length} broker transaction stock files for ${dateSuffix} (${type}, ${investorType}), ${result.stockCount} stocks processed`);
        }
      }
  ...
  private async createBrokerTransactionPerStock(data: TransactionData[], dateSuffix: string, type: TransactionType, investorType: InvestorType, progressTracker?: ProgressTracker): Promise<{ files: string[]; stockCount: number }> {
  ...
  public async generateBrokerTransactionData(_dateSuffix?: string, logId?: string | null): Promise<{ success: boolean; message: string; data?: any }> {
    ...
    try {
        ...
        updateProgress: async () => {
          if (progressTracker.logId) {
            await SchedulerLogService.updateLog(progressTracker.logId, {
              ...
              current_processing: `Processing broker transaction stock (RG/TN/NG x D/F) files...`
            });
          }
        }
      };
      ...
      for (let i = 0; i < dtFiles.length; i += BATCH_SIZE) {
        ...
        console.log(`📊 Batch ${batchNum}/${totalBatches} complete: ${totalProcessed} processed, ${totalSkipped} skipped, ${totalErrors} errors, ${batchStockCount} stocks processed`);
