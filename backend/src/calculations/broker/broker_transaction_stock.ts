...
export class BrokerTransactionStockCalculator {
   ...
   * OPTIMIZED: Skip files where broker_transaction_stock folder already exists
   */
  private async findAllDtFiles(): Promise<string[]> {
    ...
    try {
      ...
      // OPTIMIZATION: Check which dates already have broker_transaction_stock output
      console.log("🔍 Checking existing broker_transaction_stock folders to skip...");
      const filesToProcess: string[] = [];
      let skippedCount = 0;
      
      for (const file of sortedFiles) {
        const dateFolder = file.split('/')[1] || '';
        const exists = await this.checkBrokerTransactionStockExists(dateFolder);
        
        if (exists) {
          skippedCount++;
          console.log(`⏭️ Skipping ${file} - broker_transaction_stock/broker_transaction_stock_${dateFolder}/ already exists`);
        } else {
  ...
  /**
   * Create broker transaction files for each stock (pivoted - group by stock, list brokers)
  ...
  private async createBrokerTransactionPerStock(
    ...
    console.log("\nCreating broker transaction files per stock (pivoted)...");
  ...
  /**
   * Process a single DT file with broker transaction analysis (pivoted by stock)
  ...
  private async processSingleDtFile(blobName: string): Promise<{ success: boolean; dateSuffix: string; files: string[]; timing?: any }> {
    ...
    const exists = await this.checkBrokerTransactionStockExists(dateFolder);
    if (exists) {
      console.log(`⏭️ Skipping ${blobName} - broker_transaction_stock/broker_transaction_stock_${dateFolder}/ already exists (race condition check)`);
    ...
    try {
      ...
      const brokerTransactionFiles = await this.createBrokerTransactionPerStock(data, dateSuffix);
  ...
  /**
   * Main function to generate broker transaction data for all DT files (pivoted by stock)
   */
  public async generateBrokerTransactionData(_dateSuffix: string): Promise<{ success: boolean; message: string; data?: any }> {
    ...
    try {
      ...
      console.log(`✅ Broker transaction stock data analysis completed!`);
      ...
      console.log(`   📊 Broker Transaction Stock files: ${brokerTransactionStockFiles} (${totalTiming.brokerTransactionStock}s)`);
      console.log(`✅ Broker Transaction Stock Data calculation completed successfully`);
      console.log(`✅ Broker Transaction Stock Data completed in ${totalDuration}s`);
      
      return {
        success: true,
        message: `Broker transaction stock data generated successfully for ${successful}/${processed} DT files`,
        data: {
      ...
      console.error('Error generating broker transaction stock data:', error);
      return {
        success: false,
...
export default BrokerTransactionStockCalculator;
