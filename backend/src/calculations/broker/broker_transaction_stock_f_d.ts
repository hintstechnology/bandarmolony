...
export class BrokerTransactionStockFDCalculator {
  ...
  private async findAllDtFiles(): Promise<string[]> {
    ...
    try {
      ...
      // OPTIMIZATION: Check which dates already have broker_transaction_stock_d and broker_transaction_stock_f outputs (BATCH CHECKING for speed)
      console.log("🔍 Checking existing broker_transaction_stock_d/f folders to skip (batch checking - quick check: D only)...");
        ...
        for (const result of checkResults) {
          if (result.exists) {
            ...
            if (skippedCount <= 5 || skippedCount > limitedFiles.length - 5) {
              console.log(`⏭️ Skipping ${result.file} - broker_transaction_stock_d/f folders already exist for ${result.dateFolder}`);
            }
          } else {
   ...
   * OPTIMIZED: Added retry logic and quick check (only check D for speed)
   */
  private async checkBrokerTransactionStockFDExists(dateSuffix: string): Promise<boolean> {
    const maxRetries = 2;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        // Quick check: only check D type for speed (if D exists, assume F also exists)
  ...
  private async processSingleDtFile(blobName: string, progressTracker?: ProgressTracker): Promise<{ success: boolean; dateSuffix: string; files: string[]; timing?: any; stockCount?: number }> {
    ...
    // Active date sudah di-set di generateBrokerTransactionStockFDData() SETELAH pre-check
        ...
        updateProgress: async () => {
          if (progressTracker.logId) {
            await SchedulerLogService.updateLog(progressTracker.logId, {
              ...
              current_processing: `Processing broker transaction stock (F/D) files...`
            });
          }
        }
      };
