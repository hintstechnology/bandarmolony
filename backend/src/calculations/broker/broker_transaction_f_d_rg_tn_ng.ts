...
export class BrokerTransactionFDRGTNNGCalculator {
  ...
  private async checkAllCombinationsExist(dateSuffix: string): Promise<boolean> {
    ...
    const dRgExists = await this.checkBrokerTransactionFDRGTNNGExists(dateSuffix, 'RG', 'D');
    if (dRgExists) {
  ...
  private async findAllDtFiles(): Promise<string[]> {
    ...
    try {
      ...
      // OPTIMIZATION: Check which dates already have all broker_transaction combinations (BATCH CHECKING for speed)
      console.log("🔍 Checking existing broker_transaction_rg/tn/ng_d/f folders to skip (batch checking - quick check: D_RG only)...");
        ...
        for (const result of checkResults) {
          if (result.exists) {
            ...
            if (skippedCount <= 5 || skippedCount > limitedFiles.length - 5) {
              console.log(`⏭️ Skipping ${result.file} - broker_transaction_rg/tn/ng_d/f folders already exist for ${result.dateFolder}`);
            }
          } else {
  ...
  private async processSingleDtFile(blobName: string, progressTracker?: ProgressTracker): Promise<{ success: boolean; dateSuffix: string; files: string[]; timing?: any; brokerCount?: number }> {
    ...
    // Active date sudah di-set di generateBrokerTransactionFDRGTNNGData() SETELAH pre-check
        ...
        updateProgress: async () => {
          if (progressTracker.logId) {
            await SchedulerLogService.updateLog(progressTracker.logId, {
              ...
              current_processing: `Processing broker transaction (RG/TN/NG x D/F) files...`
            });
          }
        }
      };
