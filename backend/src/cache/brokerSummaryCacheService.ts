/**
 * Broker Summary Cache Service
 * 
 * Shared cache service untuk menyimpan data broker_summary CSV files
 * yang sudah di-load, sehingga tidak perlu bolak-balik memanggil Azure
 * untuk file yang sama.
 * 
 * Cache ini di-share antar semua kalkulasi yang menggunakan broker_summary data,
 * seperti Broker Summary Sector, Broker Summary IDX, dll.
 */
...
class BrokerSummaryCacheService {
  // Cache untuk raw CSV content
  ...
  // Format: 'broker_summary/broker_summary_20241021/BBCA.csv'
   ...
   * @param blobName Full blob path (e.g., 'broker_summary/broker_summary_20241021/BBCA.csv')
   */
  addActiveProcessingFile(blobName: string): void {
    if (!this.activeProcessingFiles.has(blobName)) {
      ...
      console.log(`📅 Broker Summary Cache: Added active processing file ${blobName} (total: ${this.activeProcessingFiles.size})`);
    }
  ...
  removeActiveProcessingFile(blobName: string): void {
    if (this.activeProcessingFiles.has(blobName)) {
      ...
      console.log(`📅 Broker Summary Cache: Deactivated processing file ${blobName} (remaining active: ${this.activeProcessingFiles.size})`);
    }
  }
  ...
  private autoCleanupInactiveFiles(): void {
    ...
    for (const file of filesToRemove) {
      const cached = this.rawContentCache.get(file);
      if (cached) {
        this.rawContentCache.delete(file);
    ...
    if (filesToRemove.length > 0) {
      console.log(`🧹 Auto-cleaned ${filesToRemove.length} inactive broker_summary files`);
    }
  }
  ...
  async getRawContent(blobName: string): Promise<string | null> {
    ...
    if (cached && (Date.now() - cached.timestamp) < this.CACHE_TTL) {
      ...
      console.log(`📦 Broker Summary Cache HIT: ${blobName} (${(cached.size / 1024).toFixed(2)} KB)`);
      return cached.content;
    }
    
    // Cache miss - load from Azure
    this.stats.cacheMisses++;
    console.log(`📥 Broker Summary Cache MISS: ${blobName} - loading from Azure...`);
    ...
    try {
      ...
      console.error(`❌ Error loading broker_summary ${blobName}:`, error);
      return null;
    }
  }
  ...
  private async evictOldEntries(requiredSize: number): Promise<void> {
    console.log(`🧹 Evicting old broker_summary cache entries to free ${(requiredSize / 1024 / 1024).toFixed(2)} MB...`);
    ...
    console.log(`✅ Evicted ${evictedCount} broker_summary entries, freed ${(freedSize / 1024 / 1024).toFixed(2)} MB`);
  }
   ...
   */
  clearAll(): void {
    console.log('🧹 Clearing all broker_summary caches...');
    this.rawContentCache.clear();
    this.activeProcessingFiles.clear();
    this.fileLastAccess.clear();
    this.currentCacheSize = 0;
    console.log('✅ All broker_summary caches cleared');
  }
  ...
  printStats(): void {
    ...
    console.log('\n📊 ===== BROKER SUMMARY CACHE STATISTICS =====');
  ...
  resetStats(): void {
    ...
    console.log('🔄 Broker Summary cache statistics reset');
  }
}

// Singleton instance
export const brokerSummaryCache = new BrokerSummaryCacheService();
